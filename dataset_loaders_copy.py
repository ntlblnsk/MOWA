...
class InferenceDataset(Dataset):
    def __init__(self, data_path, task_id):
        ...
        input_dir = os.path.join(self.test_path, 'input')
        self.input_images = glob.glob(os.path.join(input_dir, '*.*'))
        self.input_images.sort()

    def __getitem__(self, index):
        input_img = cv2.imread(self.input_images[index])

        input1 = input_img.astype(dtype=np.float32) / 255.0
        input1 = np.transpose(input1, [2, 0, 1])

        input2 = cv2.resize(input_img, (self.width, self.height))
        input2 = input2.astype(dtype=np.float32) / 255.0
        input2 = np.transpose(input2, [2, 0, 1])
        ...
        return {
            ...
            'mask_tensor': torch.tensor(mask),
            'task_id_tensor': torch.tensor(self.task_id, dtype=torch.int64),
            'file_name': os.path.splitext(os.path.basename(self.input_images[index]))[0]
        }
